...
*/    
    ...
    function is_sale_of_vendor($sale_id,$vendor_id)
    {
        ...
        $jsonData = $this->get_type_name_by_id('sale', $sale_id, 'product_details');
        $product_details = json_decode($jsonData,true);
        ...
        if(!$this->input->is_ajax_request()){
            ...
            if (!$user_data) {
                if ($_SERVER['HTTP_HOST'] !== 'localhost') {
                    $ip_data = '{"as":"AS24560 Bharti Airtel Ltd., Telemedia Services","city":"Ludhiana","country":"India","countryCode":"IN","isp":"Airtel Broadband","lat":30.9,"lon":75.85,"org":"Airtel Broadband","query":"122.180.85.207","region":"PB","regionName":"State of Punjab","status":"success","timezone":"Asia/Kolkata","zip":"141001"}';
	...
	function horseBreedAll()
    {
        return 'onnnnnn';
    }
    function get_states($country_id, $zone_id)
    { 
        $q = "SELECT COUNT(*) AS cnt FROM zone_area_division WHERE zone_id = '$zone_id'";
        $r = $this->db->query($q);
        $re = $r->row();

        if($re->cnt == 0) {
            
        $sql = "SELECT DISTINCT s.id, name, IF(s.id IN (SELECT state_id FROM zone_area_division WHERE country_id = '$country_id'), 1, 0) as flag FROM states AS s WHERE s.country_id = '$country_id'";
        $sql2 = "SELECT * FROM `zone_area_division` WHERE zone_id <> '$zone_id' AND country_id = '$country_id'";    
        } else {
            
        $sql = "SELECT DISTINCT s.id, name, IF(s.id IN (SELECT state_id FROM zone_area_division WHERE country_id = '$country_id' AND zone_id = '$zone_id'), 1, 0) as flag FROM states AS s";
     ...
     function get_countries($zone_id)
    {
        $q = "SELECT COUNT(*) AS cnt FROM zone_area_division WHERE zone_id = '$zone_id'";
        $r = $this->db->query($q);
        $re = $r->row();
        ...
        if($re->cnt == 0) {
            ...
            $sql = "SELECT DISTINCT c.country_id, country_name, IF(c.country_id IN (SELECT country_id FROM zone_area_division WHERE zone_id <> '$zone_id'), 2, 0) as flag FROM country AS c";
            
        } else {
            
           $sql = "SELECT DISTINCT c.country_id, country_name, IF(c.country_id IN (SELECT country_id FROM zone_area_division WHERE zone_id = '$zone_id'), 1, 0) as flag FROM country AS c ";
            
             $sql2 = "SELECT * FROM `zone_area_division` WHERE zone_id <> '$zone_id'"; 
        ...
        if(isset($sql2) && $sql2 != '') {
            ...
            foreach($rows as $kk => $rr) {
               foreach($rows2 as $tt) {
                   if($rr['country_id'] == $tt['country_id']) {
					if($rr['country_id'] != 230)
					{
						$rows[$kk]['flag'] = 2;
					}
				   }
			...
			$usa_key = array_search('230', array_column($rows, 'country_id'));
			$rows[$usa_key]['flag'] = 0;
		}
    ...
    function check_country($zone_id)
    {
        $sql = "SELECT zone_id, country_id FROM zone_area_division WHERE zone_id = '$zone_id' group by country_id";
        $query = $this->db->query($sql);
        $rows = $query->result_array();
        return $rows;
    }
    
	function zone_countries($new_country_ids, $existing_ids, $zone_id, $all_data)
	{
		$remove_countries = array_diff($existing_ids,array_diff($existing_ids, $new_country_ids));
		if(!empty($remove_countries))
		{
			foreach($remove_countries as $remove_country)
			{
				$this->db->where('zone_id', $zone_id);
				$this->db->where('country_id', $remove_country);
				$this->db->delete('zone_area_division');				
			}
		}
		if(!in_array(230,$new_country_ids) && array_key_exists('230', $all_data)){
			$this->db->where('zone_id', $zone_id);
			$this->db->where('country_id', 230);
			$this->db->delete('zone_area_division');
			foreach($all_data['230'] as $usa_state)
			{
				$data_state=array(
						 'zone_id'       => $zone_id,
						 'country_id'    => 230,
						 'state_id'      => $usa_state['state_id']
				);
				$this->db->insert('zone_area_division', $data_state);
			}
		}
		foreach($new_country_ids as $new_country_id)
		{
			if(!in_array($new_country_id, $existing_ids))
			{
				if($new_country_id == 230){
					foreach($all_data[$new_country_id] as $coun_data)
					{
						$data_state=array(
						 'zone_id'       => $zone_id,
						 'country_id'    => $new_country_id,
						 'state_id'      => $coun_data['state_id']
					);
					$this->db->insert('zone_area_division', $data_state);
					}
				}else{
					...
					$this->db->insert('zone_area_division', $data);
				}
			}
		}
	}
    
    function get_states_list($countries)
    {
        $query = $this->db->query();
    }  
	function getsiglefield($tablename,$fieldname,$fieldvalue)
	 {
		 $result = $this->db->get_where($tablename,array($fieldname=>$fieldvalue))->row();
		 return $result;
	 }
	
	function getdublefield($tablename,$fieldname1,$fieldvalue1,$fieldname2,$fieldvalue2)
	 {
		 $result = $this->db->get_where($tablename,array($fieldname1=>$fieldvalue1,$fieldname2=>$fieldvalue2))->row();
		 return $result;
	 } 
	function getfieldsval($tablename,$where)
	{
		$result = $this->db->get_where($tablename,$where)->row();
		return $result;
	}
