...
class Paypal {
   ...
   function __construct() {
      ...
      $CI=& get_instance();
      $CI->load->database();
      $type = $CI->db->get_where('business_settings',array('type'=>'paypal_type'))->row()->value;
      if($type == 'sandbox') {
   ...
   function submit_vendor_payment(){
      $form = '';           
      $form .='<form id="vendor_pay" method="post" name="paypal_form" action="'.$this->paypal_url.'">';
      foreach ($this->fields as $name => $value) {
        $form .='<input type="hidden" name="'.$name.'" value="'.$value.'"/>';
       }
      $form .='<div id="vendor_payment" class="btn-u btn-u-cust btn-block margin-bottom-20 reg_btn" style="margin-top: 10px;">Proceed to Payment</div>  
      </form>';     
      return $form;
   }
