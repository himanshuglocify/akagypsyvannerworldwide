	...
	function ajax_set_full(type,title,noty,form_id,id){
		...
		ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id+'/'+form_id,'list','form');
		sound('ajax_load');
	}
    ...
    $('#ful').on('click','.submitter', function(){
	...
	window.onpopstate = function(event) {
		...
		$("#ful").remove();
		$("#ful_up").prepend('<div id="ful"><div id="content-container"></div></div>');
		$("#content-container").html(loading);
		$("#ful").load(url);
		var pul = url.replace(base_url+'index.php/admin/','');
		if(pul == ''){
			pul = 'dashboard';
		}
		var it = $('#mainnav-menu').find("[data-ref='" + pul + "']");
	    it.closest('#mainnav-menu').find('li').each(function(){
			var now = $(this);
	    	if(now.hasClass('active-sub')){
	    		now.removeClass('active-sub');
	    		now.find('.in').removeClass('active-sub');
	    	} else {
				now.removeClass('active-link');	    		
	    	}
	    });
	    if(it.closest('ul').hasClass('collapse')){
	    	it.closest('ul').closest('li').addClass('active-sub');
	    	it.closest('li').addClass('active-link');
	    	it.closest('ul').addClass('in');
	    } else {
	...
	function form_submit(form_id,noty){ 
		...
		form.find(".required").each(function(){
            ...
            if(here.val() == ''){
				...
				else if(form_id == 'user_add' || form_id == 'user_edit'){
					
                }
				else if(form_id == 'vendor_add' || form_id == 'vendor_edit'){
			...
			if(form_id !== 'vendor_pay'){ 						
				$.ajax({
					...
					beforeSend: function() {
						...
						//alerta.fadeOut();
						alerta.html(loading1); // change submit button text
						 $('html, body').animate({
							scrollTop: 0
						}, 1000);
					},
	...
	function changeStatus(type,type_fun,action,value,id,arg,tokenname,tokenval)
	{
		var here = $(arg); // alert div for show alert message
        var form = here.closest('form');
        var can = '';
	    var msg = 'successfuly Updated';
		var prv = here.html();
		var tokenvl = tokenval;
		//return false;
		$.ajax({
                url: base_url+'index.php/'+type+'/'+type_fun+'/'+action+'/'+value+'/'+id, // form action url
				data: {'token':tokenvl}, 
                type: 'POST',
                success: function() {
                    here.fadeIn();
