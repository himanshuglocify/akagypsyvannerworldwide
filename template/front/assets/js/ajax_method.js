...
/*get horse size change*/
	
	function loadHorsesize(valu,gender_group,horsefield,tval,tval1)
	{
		var htlid = '#'+gender_group+'_horse_size_id';
		var secondsizeid = '';
		var gname = '';
		var disblebtn = '#'+gender_group+'_avalibe_id';
		/*jQuery(disblebtn).val(horse_id);*/
		
		if(gender_group == 'male')
		{
			var selid = '#female_horse_drop_id';
			secondsizeid = '#female_horse_size_id';
			 gname = 'female';
		}
		else
		{
			var selid = '#male_horse_drop_id';
			gname = 'male';
			secondsizeid = '#male_horse_size_id';
		}
		
		$.ajax({
                url: base_url+'index.php/breed/getHosefield/'+valu+'/'+gender_group, // form action url
				data:{'token':tval},
                type: 'POST', // form submit method get/post
                dataType: 'html', 
                
                success: function(msg) {
                    jQuery(htlid).html(msg);
					var cotnum = msg.length;
					
					if(cotnum>50 || msg == 'Records not found')
					{
						$.ajax({
							url: base_url+'index.php/breed/getHosefield/'+valu+'/'+gname, // form action url
							type: 'POST', // form submit method get/post
							data:{'token':tval1}, 
							dataType: 'html', 							
							success: function(data) {
							jQuery(selid).val(valu);							
							jQuery(secondsizeid).html(data);
							},
							error: function(e) {
								console.log(e)
							}
